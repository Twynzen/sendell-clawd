...
- `/usage cost` in chats: local cost summary aggregated from Sendell session logs.
- CLI: `sendell status --usage` prints a full per-provider breakdown.
- CLI: `sendell channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
