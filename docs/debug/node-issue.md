...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Sendell
---

# Node + tsx "__name is not a function" crash

## Summary
Running Sendell via Node with `tsx` fails at startup with:

```
[sendell] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `sendell status` and `gateway:watch` worked.
