...
read_when:
  - You are building a Sendell plugin
  - You need to ship a plugin config schema or debug plugin validation errors
---
# Plugin manifest (sendell.plugin.json)

Every plugin **must** ship a `sendell.plugin.json` file in the **plugin root**.
Sendell uses this manifest to validate configuration **without executing plugin
