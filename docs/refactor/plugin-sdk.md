...
Delivery:
- Publish as `@sendell/plugin-sdk` (or export from core under `sendell/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)
Scope: everything that touches core runtime behavior.
Accessed via `SendellPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: SendellConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: SendellConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: SendellConfig, agentId?: string): RegExp[];
      matchesMentionPatterns(text: string, regexes: RegExp[]): boolean;
    };
    groups: {
      resolveGroupPolicy(cfg: SendellConfig, channel: string, accountId: string, groupId: string): {
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: SendellConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: SendellConfig): string;
  };
};
```
...
### Phase 0: scaffolding
- Introduce `@sendell/plugin-sdk`.
- Add `api.runtime` to `SendellPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `sendellRuntime: ">=2026.2.0"`).
