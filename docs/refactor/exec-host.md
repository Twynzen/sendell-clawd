...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.sendell/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.sendell/node.json` (node id + pairing token).
...
Path: `~/.sendell/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.sendell/exec-approvals.sock",
...
### IPC
- Unix socket at `~/.sendell/exec-approvals.sock` (0600).
