...
Broadcast groups are evaluated after channel allowlists and group activation rules. In WhatsApp groups, this means broadcasts happen when Sendell would normally reply (for example: on mention, depending on your group settings).
...
**Result:** When Sendell would reply in this chat, it will run all three agents.
...
Workspace: /Users/pascal/sendell-alfred/
...
Workspace: /Users/pascal/sendell-baerbel/
Tools: read only
```
...
**Debug:**
```bash
tail -f ~/.sendell/logs/gateway.log | grep broadcast
```
