---
summary: "First-run onboarding flow for Sendell (macOS app)"
read_when:
...
- Writes credentials to `~/.sendell/credentials/oauth.json`
...
The app can install the global `sendell` CLI via npm/pnpm so terminal
...
On the first agent run, Sendell bootstraps a workspace (default `~/sendell`):
...
```bash
sendell webhooks gmail setup --account you@gmail.com
```
...
- `~/.sendell/credentials/oauth.json`
