...
Recommended path: use the **CLI onboarding wizard** (`sendell onboard`). It sets up:
...
- **Recommended:** Brave Search API key for web search. Easiest path:
  `sendell configure --section web` (stores `tools.web.search.apiKey`).
...
- OAuth credentials (legacy import): `~/.sendell/credentials/oauth.json`
...
If you’re hacking on Sendell itself, run from source:
...
If `sendell health` shows “no auth configured”, go back to the wizard and set OAuth/key auth — the agent won’t be able to respond without it.

Tip: `sendell status --all` is the best pasteable, read-only debug report.
Health probes: `sendell health` (or `sendell status --deep`) asks the running gateway for a health snapshot.
