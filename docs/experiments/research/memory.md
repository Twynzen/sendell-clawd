---
summary: "Research notes: offline memory system for Sendell workspaces (Markdown source-of-truth + derived index)"
read_when:
  - Designing workspace memory (~/sendell) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Sendell integration
  - Adding offline recall + reflection (retain/recall/reflect)
---

# Workspace Memory v2 (offline): research notes

Target: Sendell-style workspace (`agents.defaults.workspace`, default `~/sendell`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/sendell` as canonical human-readable memory.
...
- `memory.md` remains “small + core-ish”: the things you want Sendell to see every session.
...
Recommendation: **deep integration in Sendell**, but keep a separable core library.

### Why integrate into Sendell?
- Sendell already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `sendell memory recall "…" --k 25 --since 30d`
  - `sendell memory reflect --since 7d`
