...
- Replaces the legacy `gateway` command. CLI entry point: `sendell gateway`.
- Runs until stopped; exits non-zero on fatal errors so the supervisor restarts it.

## How to run (local)
```bash
sendell gateway --port 18789
# for full debug/trace logs in stdio:
sendell gateway --port 18789 --verbose
# if the port is busy, terminate listeners then start:
sendell gateway --force
# dev loop (auto-reload on TS changes):
pnpm gateway:watch
```
- Config hot reload watches `~/.sendell/sendell.json` (or `SENDELL_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__sendell__/canvas/` from `~/sendell/canvas`. Disable with `canvasHost.enabled=false` or `SENDELL_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `SENDELL_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `SENDELL_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `com.sendell.<profile>`
- Linux: `sendell-gateway-<profile>.service`
- Windows: `Sendell Gateway (<profile>)`

Install metadata is embedded in the service config:
- `SENDELL_SERVICE_MARKER=sendell`
- `SENDELL_SERVICE_KIND=gateway`
- `SENDELL_SERVICE_VERSION=<version>`
...
```bash
sendell --dev setup
sendell --dev gateway --allow-unconfigured
...
- `SENDELL_STATE_DIR=~/.sendell-dev`
- `SENDELL_CONFIG_PATH=~/.sendell-dev/sendell.json`
- `SENDELL_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- `browser.controlUrl=http://127.0.0.1:19003` (derived: `gateway.port+2`)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/sendell-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):
- Base port = `gateway.port` (or `SENDELL_GATEWAY_PORT` / `--port`)
- `browser.controlUrl port = base + 2` (or `SENDELL_BROWSER_CONTROL_URL` / config override)
- `canvasHost.port = base + 4` (or `SENDELL_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:
- unique `gateway.port`
- unique `SENDELL_CONFIG_PATH`
- unique `SENDELL_STATE_DIR`
- unique `agents.defaults.workspace`
- separate WhatsApp numbers (if using WA)

Service install per profile:
```bash
sendell --profile main gateway install
sendell --profile rescue gateway install
```

Example:
```bash
SENDELL_CONFIG_PATH=~/.sendell/a.json SENDELL_STATE_DIR=~/.sendell-a sendell gateway --port 19001
SENDELL_CONFIG_PATH=~/.sendell/b.json SENDELL_STATE_DIR=~/.sendell-b sendell gateway --port 19002
```
...
## Methods (initial set)
- `health` — full health snapshot (same shape as `sendell health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `sendell`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `sendell gateway install` writes `~/Library/LaunchAgents/com.sendell.gateway.plist`
    (or `com.sendell.<profile>.plist`).
  - `sendell doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are Sendell profile services.
  ...
  - Cleanup: `sendell gateway uninstall` (current service) and `sendell doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `sendell gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:
- Sendell.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `com.sendell.gateway` (or `com.sendell.<profile>`).
- To stop it cleanly, use `sendell gateway stop` (or `launchctl bootout gui/$UID/com.sendell.gateway`).
- To restart, use `sendell gateway restart` (or `launchctl kickstart -k gui/$UID/com.sendell.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `sendell gateway install` first.
  - Replace the label with `com.sendell.<profile>` when running a named profile.

## Supervision (systemd user unit)
Sendell installs a **systemd user service** by default on Linux/WSL2. We
...
`sendell gateway install` writes the user unit. `sendell doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/sendell-gateway[-<profile>].service`:
```
[Unit]
Description=Sendell Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/sendell gateway --port 18789
Restart=always
RestartSec=5
Environment=SENDELL_GATEWAY_TOKEN=
...
```
systemctl --user enable --now sendell-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/sendell-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:
```
sudo systemctl daemon-reload
sudo systemctl enable --now sendell-gateway[-<profile>].service
```
...
## CLI helpers
- `sendell gateway health|status` — request health/status over the Gateway WS.
- `sendell message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `sendell agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `sendell gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `sendell gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance
- Retire uses of `sendell gateway` and the legacy TCP control port.
