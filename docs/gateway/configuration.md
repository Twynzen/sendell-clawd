---
summary: "All configuration options for ~/.sendell/sendell.json with examples"
read_when:
  - Adding or modifying config fields
---
# Configuration 🔧

Sendell reads an optional **JSON5** config from `~/.sendell/sendell.json` (comments + trailing commas allowed).

If the file is missing, Sendell uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/sendell`). You usually only need a config to:
...
Sendell only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:
- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `sendell doctor`, `sendell logs`, `sendell health`, `sendell status`, `sendell service`, `sendell help`).
- Run `sendell doctor` to see the exact issues.
- Run `sendell doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `sendell config set`. Keep a backup of `~/.sendell/sendell.json`.
...
```bash
sendell gateway call config.get --params '{}' # capture payload.hash
sendell gateway call config.apply --params '{
...
```bash
sendell gateway call config.get --params '{}' # capture payload.hash
sendell gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/sendell" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@sendell", "reisponde"] }
      }
    ]
  },
  channels: {
...
```json5
// ~/.sendell/agents.json5
{
  defaults: { sandbox: { mode: "all", scope: "session" } },
  list: [
    { id: "main", workspace: "~/sendell" }
  ]
}
```
...
{ "$include": "/etc/sendell/base.json5" } // absolute
...
```json5
// ~/.sendell/clients/mueller/agents.json5
[
...
```json5
// ~/.sendell/clients/mueller/broadcast.json5
{
...
Sendell reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, Sendell runs your login shell and imports only the missing expected keys (never overrides).
...
Env var equivalent:
- `SENDELL_LOAD_SHELL_ENV=1`
- `SENDELL_SHELL_ENV_TIMEOUT_MS=15000`
...
Sendell stores **per-agent** auth profiles (OAuth + API keys) in:
- `<agentDir>/auth-profiles.json` (default: `~/.sendell/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:
- `~/.sendell/credentials/oauth.json` (or `$SENDELL_STATE_DIR/credentials/oauth.json`)

The embedded Pi agent maintains a runtime cache at:
- `<agentDir>/auth.json` (managed automatically; don’t edit manually)

Legacy agent dir (pre multi-agent):
- `~/.sendell/agent/*` (migrated by `sendell doctor` into `~/.sendell/agents/<defaultAgentId>/agent/*`)

Overrides:
- OAuth dir (legacy import only): `SENDELL_OAUTH_DIR`
- Agent dir (default agent root override): `SENDELL_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, Sendell imports `oauth.json` entries into `auth-profiles.json`.

Sendell also auto-syncs OAuth tokens from external CLIs into `auth-profiles.json` (when present on the gateway host):
...
credential is a setup-token. Sendell auto-migrates older configs that used
`mode: "token"`.
...
If set, Sendell derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/sendell/sendell-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/sendell/sendell.log`.
...
- The legacy single-account Baileys auth dir is migrated by `sendell doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [
      { id: "main", groupChat: { mentionPatterns: ["@sendell", "sendell", "sendell"] } }
    ]
  }
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@sendell"]
        }
      }
    ]
  }
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/sendell-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.sendell/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/sendell-home" },
      { id: "work", workspace: "~/sendell-work" }
    ]
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `sendell.json`).
...
Sendell starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["sendell-dm"]          // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {               // guild id (preferred) or slug
          slug: "friends-of-sendell",
...
Sendell starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `sendell plugins install @sendell/mattermost` (or `./extensions/mattermost` from a git checkout).
...
Sendell starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
Sendell spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, Sendell
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/sendell" } }
}
```
...
When a file exceeds this limit, Sendell logs a warning and injects a truncated
...
message envelopes). If unset, Sendell uses the host timezone at runtime.
...
`[sendell]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[sendell]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[sendell]"`, Sendell will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, Sendell generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.sendell/settings/tts.json",
      elevenlabs: {
...
`voiceAliases` lets Talk directives use friendly names (e.g. `"voice":"Sendell"`).
...
Sendell also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, Sendell currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `sendell configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:sendell`: all built-in Sendell tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.sendell/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.sendell/sandboxes",
        docker: {
          image: "sendell-sandbox:bookworm-slim",
          containerPrefix: "sendell-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
          ...
          apparmorProfile: "sendell-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "sendell-sandbox-browser:bookworm-slim",
          containerPrefix: "sendell-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
Sendell uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.sendell/agents/<agentId>/agent/models.json` or by defining the same schema inside your
Sendell config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, Sendell writes/merges a `models.json` into
`~/.sendell/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. Sendell uses
...
- Shortcut: `sendell onboard --auth-choice opencode-zen`.
...
Shortcut: `sendell onboard --auth-choice zai-api-key`.
...
Notes:
- Set `MOONSHOT_API_KEY` in the environment or use `sendell onboard --auth-choice moonshot-api-key`.
...
Notes:
- Set `KIMICODE_API_KEY` in the environment or use `sendell onboard --auth-choice kimi-code-api-key`.
...
Notes:
- Set `SYNTHETIC_API_KEY` or use `sendell onboard --auth-choice synthetic-api-key`.
...
Notes:
- Set `MINIMAX_API_KEY` environment variable or use `sendell onboard --auth-choice minimax-api`.
...
- Override the agent config root with `SENDELL_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.sendell/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.sendell/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.sendell/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, Sendell stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.sendell/skills` (workspace skills
...
from `~/.sendell/extensions`, `<workspace>/.sendell/extensions`, plus any
...
### `browser` (sendell-managed browser)

Sendell can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for sendell and expose a small loopback control server.
...
- profile color: `#FF4500` (guide-orange)
- Note: the control server is started by the running gateway (Sendell.app menubar, or `sendell gateway`).
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/sendell" }
...
- Examples: `"/ui"`, `"/sendell"`, `"/apps/sendell"`.
...
- When a connection comes from one of these IPs, Sendell uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:
- `sendell gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `SENDELL_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `SENDELL_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. Sendell
...
macOS app behavior:
- Sendell.app watches `~/.sendell/sendell.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.sendell/sendell.json` (or `SENDELL_CONFIG_PATH`) and applies changes automatically.
...
Files watched:
- `~/.sendell/sendell.json` (or `SENDELL_CONFIG_PATH`)
...
- `SENDELL_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):
- `sendell --dev …` → uses `~/.sendell-dev` + shifts ports from base `19001`
- `sendell --profile <name> …` → uses `~/.sendell-<name>` (port via config/env/flags)
...
Example:
```bash
SENDELL_CONFIG_PATH=~/.sendell/a.json \
SENDELL_STATE_DIR=~/.sendell-a \
sendell gateway --port 19001
```
...
Gmail helper config (used by `sendell webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "sendell@gmail.com",
...
- Set `SENDELL_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, Sendell defaults `serve.path` to `/` so
...
Default root: `~/sendell/canvas`  
Default port: `18793` (chosen to avoid the sendell browser CDP port `18792`)  
The server listens on the **gateway bind host** (LAN or Tailnet) so nodes can reach it.

The server:
- serves files under `canvasHost.root`
- injects a tiny live-reload client into served HTML
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__sendell/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__sendell__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
- env: `SENDELL_SKIP_CANVAS_HOST=1`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.sendell/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.sendell/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.sendell/bridge/tls/bridge-key.pem"
    }
  }
}
```
...
Controls LAN mDNS discovery broadcasts (`_sendell-gw._tcp`).
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_sendell-bridge._tcp` under `~/.sendell/dns/` using the standard discovery domain `sendell.internal.`

To make iOS/Android discover across networks (Vienna ⇄ London), pair this with:
- a DNS server on the gateway host serving `sendell.internal.` (CoreDNS is recommended)
- Tailscale **split DNS** so clients resolve `sendell.internal` via that server
