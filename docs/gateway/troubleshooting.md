---
summary: "Quick troubleshooting guide for common Sendell failures"
read_when:
  - Investigating runtime issues or failures
---
# Troubleshooting 🔧

When Sendell misbehaves, here's how to fix it.
...
|---|---|---|
| `sendell status` | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview |
| `sendell status --all` | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail | When you need to share a debug report |
| `sendell status --deep` | Runs gateway health checks (incl. provider probes; requires reachable gateway) | When “configured” doesn’t mean “working” |
| `sendell gateway probe` | Gateway discovery + reachability (local + remote targets) | When you suspect you’re probing the wrong gateway |
| `sendell channels status --probe` | Asks the running gateway for channel status (and optionally probes) | When gateway is reachable but channels misbehave |
| `sendell gateway status` | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error | When the service “looks loaded” but nothing runs |
| `sendell logs --follow` | Live logs (best signal for runtime issues) | When you need the actual failure reason |

**Sharing output:** prefer `sendell status --all` (it redacts tokens). If you paste `sendell status`, consider setting `SENDELL_SHOW_SECRETS=0` first (token previews).
...
- Or paste a setup-token on the **gateway host**:
  ```bash
  sendell models auth setup-token --provider anthropic
  ```
...
```bash
sendell models auth paste-token --provider anthropic
sendell models status
```

**If you want to keep OAuth reuse:**
log in with Claude Code CLI on the gateway host, then run `sendell models status`
to sync the refreshed token into Sendell’s auth store.
...
**Logs:**
- Preferred: `sendell logs --follow`
- File logs (always): `/tmp/sendell/sendell-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$SENDELL_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u sendell-gateway[-<profile>].service -n 200 --no-pager`
...
- Or set it directly:
  ```bash
  sendell config set gateway.mode local
  ```

**If you meant to run a remote Gateway instead:**
- Set a remote URL and keep `gateway.mode=remote`:
  ```bash
  sendell config set gateway.mode remote
  sendell config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `sendell setup` to create a starter config, then rerun
the gateway.
...
variables like `DISPLAY` should live in `~/.sendell/.env` (loaded early by the
gateway).
...
service was installed with Bun or a version-managed Node path, run `sendell doctor`
...
- then run `sendell sandbox recreate --agent <id>` (or `--all`)
...
**Check:**
- `gateway.mode` must be `local` for `sendell gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `sendell gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `sendell gateway status` and `sendell doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
- Non-loopback binds (`lan`/`tailnet`/`custom`, or `auto` when loopback is unavailable) require auth:
  `gateway.auth.token` (or `SENDELL_GATEWAY_TOKEN`).
- `gateway.remote.token` is for remote CLI calls only; it does **not** enable local auth.
- `gateway.token` is ignored; use `gateway.auth.token`.

**If `sendell gateway status` shows a config mismatch**
...
- Fix: rerun `sendell gateway install --force` from the same `--profile` / `SENDELL_STATE_DIR` you want the service to use.

**If `sendell gateway status` reports service config issues**
- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `sendell doctor` to update it (or `sendell gateway install --force` for a full rewrite).
...
- Fix: set `gateway.auth.mode` + `gateway.auth.token` (or export `SENDELL_GATEWAY_TOKEN`) and restart the service.

**If `sendell gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/sendell` on disk.
...
Symptoms: `pwd` or file tools show `~/.sendell/sandboxes/...` even though you
...
Sendell intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `sendell models list` or
  `sendell models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${SENDELL_CONFIG_PATH:-$HOME/.sendell/sendell.json}"
```
...
tail -f "$(ls -t /tmp/sendell/sendell-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
sendell logs --follow | grep "pairing request"
```
...
- ❌ `@sendell` + image
- ✅ `@sendell check this` + image

### Session Not Resuming

**Check 1:** Is the session file there?
```bash
ls -la ~/.sendell/agents/<agentId>/sessions/
```
...
sendell logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
sendell gateway --verbose
```

If you’re logged out / unlinked:

```bash
sendell channels logout
trash "${SENDELL_STATE_DIR:-$HOME/.sendell}/credentials" # if logout can't cleanly remove everything
sendell channels login --verbose       # re-scan QR
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/sendell/sendell-*.log | head -1)" | tail -20
```

### High Memory Usage

Sendell keeps conversation history in memory.
...
Sendell now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
Notes:
- `sendell doctor` reports every invalid entry.
- `sendell doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `sendell logs`, `sendell health`, `sendell status`, `sendell gateway status`, and `sendell gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/sendell/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `sendell models status` (CLI).
...
```bash
curl -fsSL https://sendell.bot/install.sh | bash -s -- --install-method git --no-onboard
```
...
5) Run `sendell channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. Sendell scrubs/normalizes tool
...
```bash
tccutil reset All com.sendell.mac.debug
```

**Fix 2: Force New Bundle ID**
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/sendell/sendell/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
# Or: launchctl bootout gui/$UID/com.sendell.gateway (replace with com.sendell.<profile> if needed)
```
...
Ensure the global `sendell` CLI is installed and matches the app version:
```bash
sendell --version
npm install -g sendell@<version>
```
...
#   ${SENDELL_CONFIG_PATH:-$HOME/.sendell/sendell.json} -> { logging: { level: "trace" } }
#
# Then run verbose commands to mirror debug output to stdout:
sendell gateway --verbose
sendell channels login --verbose
```
...
| Log | Location |
|-----|----------|
| Gateway file logs (structured) | `/tmp/sendell/sendell-YYYY-MM-DD.log` (or `logging.file`) |
| Gateway service logs (supervisor) | macOS: `$SENDELL_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.sendell/logs/...`; profiles use `~/.sendell-<profile>/logs/...`)<br />Linux: `journalctl --user -u sendell-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "Sendell Gateway (<profile>)" /V /FO LIST` |
| Session files | `$SENDELL_STATE_DIR/agents/<agentId>/sessions/` |
| Media cache | `$SENDELL_STATE_DIR/media/` |
| Credentials | `$SENDELL_STATE_DIR/credentials/` |
...
sendell gateway status --deep

# Is the gateway reachable?
sendell health --json
# If it fails, rerun with connection details:
sendell health --verbose
...
tail -20 /tmp/sendell/sendell-*.log
```
...
# sendell gateway uninstall

trash "${SENDELL_STATE_DIR:-$HOME/.sendell}"
sendell channels login         # re-pair WhatsApp
sendell gateway restart           # or: sendell gateway
```
...
1. Check logs first: `/tmp/sendell/` (default: `sendell-YYYY-MM-DD.log`, or your configured `logging.file`)
