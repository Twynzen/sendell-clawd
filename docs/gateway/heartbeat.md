...
- During heartbeat runs, Sendell treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `sendell sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Sendell skips the heartbeat run entirely (no model call).
...
```bash
sendell system event --text "Check for urgent follow-ups" --mode now
```
