...
- **gateway host** → `sendell` process on the gateway machine
...
`~/.sendell/exec-approvals.json` directly.

CLI: `sendell approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
