---
summary: "Direct `sendell agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
---
# `sendell agent` (direct agent runs)

`sendell agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `sendell message --target`).
...
```bash
sendell agent --to +15555550123 --message "status update"
sendell agent --agent ops --message "Summarize logs"
...
sendell agent --to +15555550123 --message "Summon reply" --deliver
