...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `sendell sandbox explain`.
...
Legacy `agent.*` configs are migrated by `sendell doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ```exec
   docker ps --filter "label=sendell.sandbox=1"
   ```
...
4. **Monitor logs:**
   ```exec
   tail -f "${SENDELL_STATE_DIR:-$HOME/.sendell}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
