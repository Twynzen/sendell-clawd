...
Tlon is a decentralized messenger built on Urbit. Sendell connects to your Urbit ship and can
...
```bash
sendell plugins install @sendell/tlon
```

Local checkout (when running from a git repo):

```bash
sendell plugins install ./extensions/tlon
```
...
Use these with `sendell message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Sendell replies in-thread.
