...
Matrix is an open, decentralized messaging protocol. Sendell connects as a Matrix **user**
...
```bash
sendell plugins install @sendell/matrix
```

Local checkout (when running from a git repo):

```bash
sendell plugins install ./extensions/matrix
```
...
1) Install the Matrix plugin:
   - From npm: `sendell plugins install @sendell/matrix`
   - From a local checkout: `sendell plugins install ./extensions/matrix`
...
3) Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Sendell calls the same
     login endpoint, stores the access token in `~/.sendell/credentials/matrix/credentials.json`,
...
- On first connection, Sendell requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Sendell logs a warning.
...
`~/.sendell/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `sendell pairing list matrix`
  - `sendell pairing approve matrix <CODE>`
...
- On startup, Sendell resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
