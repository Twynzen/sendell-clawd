...
```bash
sendell plugins install @sendell/nextcloud-talk
```

Local checkout (when running from a git repo):
```bash
sendell plugins install ./extensions/nextcloud-talk
```
...
2) On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "Sendell" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `sendell pairing list nextcloud-talk`
  - `sendell pairing approve nextcloud-talk <CODE>`
