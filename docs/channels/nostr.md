...
read_when:
  - You want Sendell to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Sendell to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`sendell onboard`) and `sendell channels add` list optional channel plugins.
...
```bash
sendell plugins install @sendell/nostr
```

Use a local checkout (dev workflows):

```bash
sendell plugins install --link <path-to-sendell>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "sendell@example.com",
        "lud16": "sendell@example.com"
      }
    }
  }
}
