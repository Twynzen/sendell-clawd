---
summary: "Run Sendell Gateway on exe.dev (VM + HTTPS proxy) for remote access"
read_when:
...
Goal: Sendell Gateway running on an exe.dev VM, reachable from your laptop via:
...
1) Create VM → install Node 22 → install Sendell  
2) Run `sendell onboard --install-daemon`  
...
Tip: keep this VM **stateful**. Sendell stores state under `~/.sendell/` and `~/sendell/`.
...
```bash
ssh -N -L 18789:127.0.0.1:18789 sendell.exe.xyz
```
...
```bash
export SENDELL_GATEWAY_TOKEN="$(openssl rand -hex 32)"
sendell gateway --bind lan --port 8080 --token "$SENDELL_GATEWAY_TOKEN"
```

For service runs, persist it in `~/.sendell/sendell.json`:
...
Notes:
- Non-loopback binds require `gateway.auth.token` (or `SENDELL_GATEWAY_TOKEN`).
- `gateway.remote.token` is only for remote CLI calls; it does not enable local auth.

Then point exe.dev’s proxy at `8080` (or whatever port you chose) and open your VM’s HTTPS URL:

```bash
ssh exe.dev share port sendell 8080
```

Open:
- `https://sendell.exe.xyz/`
...
On Linux, Sendell uses a systemd **user** service. After `--install-daemon`, verify:

```bash
systemctl --user status sendell-gateway[-<profile>].service
```
