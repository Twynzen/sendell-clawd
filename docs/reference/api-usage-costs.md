...
Sendell features that can generate provider usage or paid API calls.
...
- `sendell status --usage` and `sendell channels list` show provider **usage windows**
...
Sendell can pick up credentials from:
...
- `sendell status --usage`
- `sendell models status --json`
...
### 8) Model scan / probe
`sendell models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
