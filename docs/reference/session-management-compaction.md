...
This document explains how Sendell manages sessions end-to-end:
...
Sendell is designed around a single **Gateway process** that owns session state.
...
Sendell persists sessions in two layers:
...
- Transcripts: `~/.sendell/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

Sendell resolves these via `src/config/sessions.ts`.
...
Sendell intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (Sendell consumes the events, but Pi decides when to compact).
...
Sendell also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Sendell bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Sendell leaves it alone.
...
- `sendell status` (CLI)
- `sendell sessions` / `sessions --json`
...
Sendell supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:
- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- Sendell strips/suppresses this in the delivery layer.

As of `2026.1.10`, Sendell also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
Sendell uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Sendell’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `sendell status`.
