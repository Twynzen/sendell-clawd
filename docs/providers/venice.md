---
summary: "Use Venice AI privacy-focused models in Sendell"
read_when:
  - You want privacy-focused inference in Sendell
...
```bash
sendell onboard --auth-choice venice-api-key
```
...
```bash
sendell chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Sendell shows all available Venice models. Pick based on your needs:
...
```bash
sendell models set venice/claude-opus-45
sendell models set venice/llama-3.3-70b
```

List all available models:

```bash
sendell models list | grep venice
```

## Configure via `sendell configure`

1. Run `sendell configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Sendell automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
sendell chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
sendell chat --model venice/claude-opus-45

# Use uncensored model
sendell chat --model venice/venice-uncensored

# Use vision model with image
sendell chat --model venice/qwen3-vl-235b-a22b

# Use coding model
sendell chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `sendell models list` to see currently available models. Some models may be temporarily offline.
