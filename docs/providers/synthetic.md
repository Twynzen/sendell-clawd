---
summary: "Use Synthetic's Anthropic-compatible API in Sendell"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Sendell registers it as the
...
```bash
sendell onboard --auth-choice synthetic-api-key
```
...
Note: Sendell's Anthropic client appends `/v1` to the base URL, so use
