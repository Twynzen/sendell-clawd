...
read_when:
  - You installed Sendell but `sendell` is “command not found”
...
Sendell’s runtime baseline is **Node 22+**.

If you can run `npm install -g sendell@latest` but later see `sendell: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
