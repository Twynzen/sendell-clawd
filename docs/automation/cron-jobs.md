...
- Jobs persist under `~/.sendell/cron/` so restarts don’t lose schedules.
...
## Storage & history
- Job store: `~/.sendell/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.sendell/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
sendell cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
sendell cron edit <jobId> --agent ops
sendell cron edit <jobId> --clear-agent
```
```
...
```bash
sendell system event --mode now --text "Next heartbeat: check battery."
```

## Gateway API surface
- `cron.list`, `cron.status`, `cron.add`, `cron.update`, `cron.remove`
- `cron.run` (force or due), `cron.runs`
For immediate system events without a job, use [`sendell system event`](/cli/system).

## Troubleshooting

### “Nothing runs”
- Check cron is enabled: `cron.enabled` and `SENDELL_SKIP_CRON`.
