...
You can also set `SENDELL_VERBOSE=1` instead of the flag.
...
### `docs.sendell.bot` shows an SSL error (Comcast/Xfinity)

Some Comcast/Xfinity connections block `docs.sendell.bot` via Xfinity Advanced Security.
Disable Advanced Security or add `docs.sendell.bot` to the allowlist, then retry.
...
- Check the allowlist: `sendell config get agents.defaults.models`
...
If you can, include the relevant log tail from `sendell logs --follow`.
