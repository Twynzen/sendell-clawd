...
const compiler = env.SENDELL_TS_COMPILER === "tsc" ? "tsc" : "tsgo";
...
const shouldBuild = () => {
  if (env.SENDELL_FORCE_BUILD === "1") return true;
...
const logRunner = (message) => {
  if (env.SENDELL_RUNNER_LOG === "0") return;
  process.stderr.write(`[sendell] ${message}\n`);
};
