...
import type { SendellApp } from "./app";
...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") await loadPresence(host as unknown as SendellApp);
  if (host.tab === "sessions") await loadSessions(host as unknown as SendellApp);
  if (host.tab === "cron") await loadCron(host);
  if (host.tab === "skills") await loadSkills(host as unknown as SendellApp);
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as SendellApp);
    await loadDevices(host as unknown as SendellApp);
    await loadConfig(host as unknown as SendellApp);
    await loadExecApprovals(host as unknown as SendellApp);
  }
  if (host.tab === "chat") {
  ...
  if (host.tab === "config") {
    await loadConfigSchema(host as unknown as SendellApp);
    await loadConfig(host as unknown as SendellApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as SendellApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as SendellApp, { reset: true });
    scheduleLogsScroll(
...
export function inferBasePath() {
  ...
  const configured = window.__SENDELL_CONTROL_UI_BASE_PATH__;
...
export async function loadOverview(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as SendellApp, false),
    loadPresence(host as unknown as SendellApp),
    loadSessions(host as unknown as SendellApp),
    loadCronStatus(host as unknown as SendellApp),
    loadDebug(host as unknown as SendellApp),
  ]);
}

export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as SendellApp, true),
    loadConfigSchema(host as unknown as SendellApp),
    loadConfig(host as unknown as SendellApp),
  ]);
}
...
export async function loadCron(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as SendellApp, false),
    loadCronStatus(host as unknown as SendellApp),
    loadCronJobs(host as unknown as SendellApp),
  ]);
}
