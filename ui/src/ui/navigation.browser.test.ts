...
import { SendellApp } from "./app";
import "../styles.css";

const originalConnect = SendellApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("sendell-app") as SendellApp;
...
beforeEach(() => {
  SendellApp.prototype.connect = () => {
    // no-op: avoid real gateway WS connections in browser tests
  };
  window.__SENDELL_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});

afterEach(() => {
  SendellApp.prototype.connect = originalConnect;
  window.__SENDELL_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/sendell/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/sendell");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/sendell/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__SENDELL_CONTROL_UI_BASE_PATH__ = "/sendell";
    const app = mountApp("/sendell/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/sendell");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/sendell/sessions");
  });
