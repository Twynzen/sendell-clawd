...
describe("auth-profiles (chutes)", () => {
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) delete process.env.SENDELL_AGENT_DIR;
    else process.env.SENDELL_AGENT_DIR = previousAgentDir;
  ...
  it("refreshes expired Chutes OAuth credentials", async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "sendell-chutes-"));
    process.env.SENDELL_STATE_DIR = tempDir;
    process.env.SENDELL_AGENT_DIR = path.join(tempDir, "agents", "main", "agent");
