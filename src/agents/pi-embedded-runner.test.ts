...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sendell-embedded-agent-"));
...
const ensureModels = (cfg: SendellConfig) => ensureSendellModelsJson(cfg, agentDir) as unknown;
