...
import { createSendellCodingTools } from "./pi-tools.js";
...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("sendell-ws-", async (workspaceDir) => {
      await withTempDir("sendell-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createSendellCodingTools({ workspaceDir });
    ...
    await withTempDir("sendell-ws-", async (workspaceDir) => {
      await withTempDir("sendell-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello sendell");
        } finally {
          process.chdir(prevCwd);
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("sendell-ws-", async (workspaceDir) => {
      await withTempDir("sendell-override-", async (overrideDir) => {
        const tools = createSendellCodingTools({ workspaceDir });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("sendell-sandbox-", async (sandboxDir) => {
      await withTempDir("sendell-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "sendell-sbx-test",
        ...
        const tools = createSendellCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
