...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "sendell.sandbox=1");
  args.push("--label", `sendell.sessionKey=${params.scopeKey}`);
  args.push("--label", `sendell.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `sendell.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  const result = await execDocker(
    ["inspect", "-f", '{{ index .Config.Labels "sendell.configHash" }}', containerName],
    { allowFailure: true },
  );
  if (result.code !== 0) return null;
...
function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`sendell sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`sendell sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("sendell sandbox recreate --all");
}
