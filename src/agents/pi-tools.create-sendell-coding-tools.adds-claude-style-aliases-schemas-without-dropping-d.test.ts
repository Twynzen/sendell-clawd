...
const defaultTools = createSendellCodingTools();

describe("createSendellCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "sendell-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    const tools = createSendellCodingTools();
    ...
    try {
      ...
      const contents = "Hello from sendell read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "sendell-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "sendell-workspace"),
    ...
    const tools = createSendellCodingTools({ sandbox });
