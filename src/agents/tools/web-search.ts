...
type WebSearchConfig = NonNullable<SendellConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: SendellConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("sendell configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.sendell.bot/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://sendell.com",
      "X-Title": "Sendell Web Search",
    },
    body: JSON.stringify({
      model: params.model,
      messages: [
