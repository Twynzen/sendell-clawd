// Strip visually hidden content from web_fetch HTML to prevent indirect
// prompt injection attacks via CSS-hidden elements and invisible Unicode.
...
function isStyleHidden(style: string): boolean {
  ...
  // clip-path with positive percentage inset() hides content
  ...
  // transform: scale(0) or large translateX/Y
...
function shouldRemoveElement(element: Element): boolean {
  // Non-content tags that should always be stripped
