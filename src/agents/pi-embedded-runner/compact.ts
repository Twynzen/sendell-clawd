...
export async function compactEmbeddedPiSessionDirect(
  ...
  const agentDir = params.agentDir ?? resolveSendellAgentDir();
  await ensureSendellModelsJson(params.config, agentDir);
  ...
  try {
    ...
    const toolsRaw = createSendellCodingTools({
      exec: {
    ...
    const docsPath = await resolveSendellDocsPath({
      workspaceDir: effectiveWorkspace,
