...
/**
 * Cleanup suspended Sendell CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: SendellConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "sendell-cli-images-"));
  const paths: string[] = [];
