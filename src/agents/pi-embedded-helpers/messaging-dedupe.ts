...
// ---------------------------------------------------------------------------
// Cross-turn dedup cache (port of openclaw 6bee86e61)
// ---------------------------------------------------------------------------
 ...
 * the first 200 normalised chars combined with the total length and a
 * simple 53-bit FNV-1a-inspired numeric hash of the full string. This
 * avoids false positives when two responses share the same opening
 * paragraph but diverge later.
 */
...
export function isRecentlyDelivered(
  ...
  if (!hash || hash.length < MIN_DUPLICATE_TEXT_LENGTH) return false;
...
export function recordDeliveredText(
  ...
  const hash = buildDeliveredTextHash(text);
  if (!hash || hash.length < MIN_DUPLICATE_TEXT_LENGTH) return;
  const currentTime = now ?? Date.now();
