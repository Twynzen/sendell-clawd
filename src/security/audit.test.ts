...
describe("security audit", () => {
  ...
  it("flags remote browser control without token as critical", async () => {
    const prev = process.env.SENDELL_BROWSER_CONTROL_TOKEN;
    delete process.env.SENDELL_BROWSER_CONTROL_TOKEN;
    try {
      const cfg: SendellConfig = {
        browser: {
      ...
      if (prev === undefined) delete process.env.SENDELL_BROWSER_CONTROL_TOKEN;
      else process.env.SENDELL_BROWSER_CONTROL_TOKEN = prev;
    }
  });
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: SendellConfig = { session: { dmScope: "main" } };
  ...
  it("flags Discord native commands without a guild user allowlist", async () => {
    const prevStateDir = process.env.SENDELL_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "sendell-security-audit-discord-"));
    process.env.SENDELL_STATE_DIR = tmp;
    ...
    try {
      ...
      if (prevStateDir == null) delete process.env.SENDELL_STATE_DIR;
      else process.env.SENDELL_STATE_DIR = prevStateDir;
    }
  });
  ...
  it("does not flag Discord slash commands when dm.allowFrom includes a Discord snowflake id", async () => {
    ...
    const tmp = await fs.mkdtemp(
      path.join(os.tmpdir(), "sendell-security-audit-discord-allowfrom-snowflake-"),
    );
    process.env.SENDELL_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
  ...
  it("flags Discord slash commands when access-group enforcement is disabled and no users allowlist exists", async () => {
    const prevStateDir = process.env.SENDELL_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "sendell-security-audit-discord-open-"));
    process.env.SENDELL_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: SendellConfig = {
  ...
  it("flags Slack slash commands without a channel users allowlist", async () => {
    const prevStateDir = process.env.SENDELL_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "sendell-security-audit-slack-"));
    process.env.SENDELL_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: SendellConfig = {
  ...
  it("flags Slack slash commands when access-group enforcement is disabled", async () => {
    const prevStateDir = process.env.SENDELL_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "sendell-security-audit-slack-open-"));
    process.env.SENDELL_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: SendellConfig = {
  ...
  it("flags Telegram group commands without a sender allowlist", async () => {
    const prevStateDir = process.env.SENDELL_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "sendell-security-audit-telegram-"));
    process.env.SENDELL_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: SendellConfig = {
  ...
  it("adds a warning when deep probe fails", async () => {
    const cfg: SendellConfig = { gateway: { mode: "local" } };
  ...
  it("warns when hooks token reuses the gateway env token", async () => {
    const prevToken = process.env.SENDELL_GATEWAY_TOKEN;
    process.env.SENDELL_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: SendellConfig = {
    ...
    try {
      ...
      if (prevToken === undefined) delete process.env.SENDELL_GATEWAY_TOKEN;
      else process.env.SENDELL_GATEWAY_TOKEN = prevToken;
    }
  });
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await runSecurityAudit({
      ...
      includeFilesystem: false,
      includeChannelSecurity: false,
      stateDir: "/Users/test/Dropbox/.sendell",
      configPath: "/Users/test/Dropbox/.sendell/sendell.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "sendell-security-audit-"));
    const stateDir = path.join(tmp, "state");
    ...
    const configPath = path.join(stateDir, "sendell.json");
    await fs.writeFile(configPath, `{ "$include": "./extra.json5" }\n`, "utf-8");
    await fs.chmod(configPath, 0o600);

    const cfg: SendellConfig = { logging: { redactSensitive: "off" } };
    const res = await runSecurityAudit({
      config: cfg,
      includeFilesystem: true,
      includeChannelSecurity: false,
  ...
  it("flags extensions without plugins.allow", async () => {
    ...
    try {
      ...
      const res = await runSecurityAudit({
        ...
        includeChannelSecurity: false,
        stateDir,
        configPath: path.join(stateDir, "sendell.json"),
      });
  ...
  describe("maybeProbeGateway auth selection", () => {
    const originalEnvToken = process.env.SENDELL_GATEWAY_TOKEN;
    const originalEnvPassword = process.env.SENDELL_GATEWAY_PASSWORD;
    ...
    afterEach(() => {
      if (originalEnvToken == null) {
        ...
        process.env.SENDELL_GATEWAY_TOKEN = originalEnvToken;
      }
      if (originalEnvPassword == null) {
        delete process.env.SENDELL_GATEWAY_PASSWORD;
      } else {
        process.env.SENDELL_GATEWAY_PASSWORD = originalEnvPassword;
      }
    });
    ...
    it("prefers env token over local config token", async () => {
      process.env.SENDELL_GATEWAY_TOKEN = "env-token";
      let capturedAuth: { token?: string; password?: string } | undefined;
      const cfg: SendellConfig = {
        gateway: {
          mode: "local",
    ...
    it("prefers env password over remote password", async () => {
      process.env.SENDELL_GATEWAY_PASSWORD = "env-pass";
      let capturedAuth: { token?: string; password?: string } | undefined;
      const cfg: SendellConfig = {
        gateway: {
          mode: "remote",
