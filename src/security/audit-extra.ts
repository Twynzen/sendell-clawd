...
import type { SendellConfig, ConfigFileSnapshot } from "../config/config.js";
...
function summarizeGroupPolicy(cfg: SendellConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
export function collectAttackSurfaceSummaryFindings(cfg: SendellConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep SENDELL_STATE_DIR on a local-only volume and re-run "${formatCliCommand("sendell security audit --fix")}".`,
    });
  }
  return findings;
}
...
export function collectSecretsInConfigFindings(cfg: SendellConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer SENDELL_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
  ...
  if (browserToken && !looksLikeEnvRef(browserToken)) {
    findings.push({
      ...
      remediation:
        "Prefer SENDELL_BROWSER_CONTROL_TOKEN (env) and remove browser.controlToken from disk.",
    });
  }
...
export function collectHooksHardeningFindings(cfg: SendellConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  ...
  const browserToken =
    typeof cfg.browser?.controlToken === "string" && cfg.browser.controlToken.trim()
      ...
      : process.env.SENDELL_BROWSER_CONTROL_TOKEN?.trim() || null;
...
function collectModels(cfg: SendellConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
export function collectModelHygieneFindings(cfg: SendellConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function hasWebSearchKey(cfg: SendellConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: SendellConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: SendellConfig): boolean {
  const enabled = cfg.tools?.web?.fetch?.enabled;
  if (enabled === false) return false;
  return true;
}

function isBrowserEnabled(cfg: SendellConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: SendellConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
...
export function collectExposureMatrixFindings(cfg: SendellConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
