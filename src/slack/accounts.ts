...
export function listSlackAccountIds(cfg: SendellConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSlackAccountId(cfg: SendellConfig): string {
...
function mergeSlackAccountConfig(cfg: SendellConfig, accountId: string): SlackAccountConfig {
...
export function listEnabledSlackAccounts(cfg: SendellConfig): ResolvedSlackAccount[] {
