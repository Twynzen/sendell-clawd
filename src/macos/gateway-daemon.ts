...
declare const __SENDELL_VERSION__: string;
...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `sendell --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.SENDELL_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ?? process.env.SENDELL_GATEWAY_BIND ?? cfg.gateway?.bind ?? "loopback";
  const bind =
    bindRaw === "loopback" ||
    bindRaw === "lan" ||
    bindRaw === "auto" ||
