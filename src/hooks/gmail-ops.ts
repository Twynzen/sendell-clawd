...
export async function runGmailSetup(opts: GmailSetupOptions) {
  ...
  if (!configSnapshot.valid) {
    throw new Error(`Config invalid: ${CONFIG_PATH_SENDELL}`);
  }
  ...
  const nextConfig: SendellConfig = {
    ...baseConfig,
    hooks: {
      ...baseConfig.hooks,
      enabled: true,
  ...
  defaultRuntime.log(`- config: ${displayPath(CONFIG_PATH_SENDELL)}`);
  defaultRuntime.log(`Next: ${formatCliCommand("sendell webhooks gmail run")}`);
}
