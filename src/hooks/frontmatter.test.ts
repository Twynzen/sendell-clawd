...
`;
    ...
    expect(parsed.sendell.emoji).toBe("💾");
...
`;
    ...
    const parsed = JSON.parse(result.metadata as string);
    expect(parsed.sendell.emoji).toBe("📝");
    expect(parsed.sendell.events).toEqual(["command"]);
    expect(parsed.sendell.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.sendell.install[0].kind).toBe("bundled");
  });
...
describe("resolveSendellMetadata", () => {
  it("extracts sendell metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveSendellMetadata(frontmatter);
  ...
  it("returns undefined when sendell key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        sendell: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Sendell" },
            { id: "npm", kind: "npm", package: "@sendell/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@sendell/hook");
  });
...
`;
    ...
    const sendell = resolveSendellMetadata(frontmatter);
    expect(sendell).toBeDefined();
    expect(sendell?.emoji).toBe("💾");
    expect(sendell?.events).toEqual(["command:new"]);
    expect(sendell?.requires?.config).toEqual(["workspace.dir"]);
    expect(sendell?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const sendell = resolveSendellMetadata(frontmatter);
    expect(sendell?.emoji).toBe("disk");
    expect(sendell?.events).toEqual(["command:new"]);
  });
});
