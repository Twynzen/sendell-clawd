...
import type { SendellPluginApi } from "../plugins/types.js";
...
function resolveHookDir(api: SendellPluginApi, dir: string): string {
  if (path.isAbsolute(dir)) return dir;
  return path.resolve(path.dirname(api.source), dir);
}

function normalizePluginHookEntry(api: SendellPluginApi, entry: HookEntry): HookEntry {
  return {
    ...entry,
    hook: {
      ...entry.hook,
    ...
    sendell: {
      ...entry.sendell,
      hookKey: entry.sendell?.hookKey ?? `${api.id}:${entry.hook.name}`,
      events: entry.sendell?.events ?? [],
    },
  };
}
...
async function loadHookHandler(
  entry: HookEntry,
  api: SendellPluginApi,
): Promise<InternalHookHandler | null> {
  try {
    const url = pathToFileURL(entry.hook.handlerPath).href;
    const cacheBustedUrl = `${url}?t=${Date.now()}`;
    const mod = (await import(cacheBustedUrl)) as Record<string, unknown>;
    const exportName = entry.sendell?.export ?? "default";
...
export async function registerPluginHooksFromDir(
  ...
  for (const entry of hooks) {
    ...
    const events = normalizedEntry.sendell?.events ?? [];
    if (events.length === 0) {
