...
function resolveHookKey(entry: HookEntry): string {
  return entry.sendell?.hookKey ?? entry.hook.name;
}

function normalizeInstallOptions(entry: HookEntry): HookInstallOption[] {
  const install = entry.sendell?.install ?? [];
...
function buildHookStatus(
  ...
  const managedByPlugin = entry.hook.source === "sendell-plugin";
  const disabled = managedByPlugin ? false : hookConfig?.enabled === false;
  const always = entry.sendell?.always === true;
  const emoji = entry.sendell?.emoji ?? entry.frontmatter.emoji;
  const homepageRaw =
    entry.sendell?.homepage ??
    entry.frontmatter.homepage ??
    entry.frontmatter.website ??
    entry.frontmatter.url;
  const homepage = homepageRaw?.trim() ? homepageRaw.trim() : undefined;
  const events = entry.sendell?.events ?? [];
  ...
  const requiredOs = entry.sendell?.os ?? [];
