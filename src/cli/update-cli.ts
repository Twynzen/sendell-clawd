...
const STEP_LABELS: Record<string, string> = {
  ...
  "sendell doctor": "Running doctor checks",
...
const UPDATE_QUIPS = [
  ...
  "The guide has molted. Harder shell, sharper claws.",
...
const SENDELL_REPO_URL = "https://github.com/sendell/sendell.git";
const DEFAULT_GIT_DIR = path.join(os.homedir(), "sendell");

function normalizeTag(value?: string | null): string | null {
  if (!value) return null;
  const trimmed = value.trim();
  if (!trimmed) return null;
  return trimmed.startsWith("sendell@") ? trimmed.slice("sendell@".length) : trimmed;
}
...
async function isSendellPackage(root: string): Promise<boolean> {
  try {
    const raw = await fs.readFile(path.join(root, "package.json"), "utf-8");
    const parsed = JSON.parse(raw) as { name?: string };
    return parsed?.name === "sendell";
  } catch {
    return false;
  }
}
...
function resolveGitInstallDir(): string {
  const override = process.env.SENDELL_GIT_DIR?.trim();
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", SENDELL_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `SENDELL_GIT_DIR points at a non-git directory: ${params.dir}. Set SENDELL_GIT_DIR to an empty folder or a sendell checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isSendellPackage(params.dir))) {
    throw new Error(`SENDELL_GIT_DIR does not look like a sendell checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  const root =
    (await resolveSendellPackageRoot({
  ...
  defaultRuntime.log(theme.heading("Sendell update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating Sendell..."));
    defaultRuntime.log("");
  }
  ...
  if (switchToPackage) {
    ...
    const updateStep = await runUpdateStep({
      name: "global update",
      argv: globalInstallArgs(manager, `sendell@${tag}`),
      timeoutMs: timeoutMs ?? 20 * 60_000,
      progress,
    });
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this Sendell install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${formatCliCommand("sendell doctor")}\` and \`${formatCliCommand("sendell gateway restart")}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted("Examples: `npm i -g sendell@latest` or `pnpm add -g sendell@latest`"),
      );
    }
    defaultRuntime.exit(0);
    return;
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.SENDELL_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.SENDELL_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${formatCliCommand("sendell gateway restart")}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${formatCliCommand("sendell doctor")}\`, then \`${formatCliCommand("sendell gateway restart")}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${formatCliCommand("sendell gateway restart")}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `sendell update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `SENDELL_GIT_DIR points at a non-git directory: ${gitDir}. Set SENDELL_GIT_DIR to an empty folder or a sendell checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via SENDELL_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || ok === false) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Sendell to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["sendell update", "Update a source checkout (git)"],
        ["sendell update --channel beta", "Switch to beta channel (git + npm)"],
        ["sendell update --channel dev", "Switch to dev channel (git + npm)"],
        ["sendell update --tag beta", "One-off update to a dist-tag or version"],
        ["sendell update --no-restart", "Update without restarting the service"],
        ["sendell update --json", "Output result as JSON"],
        ["sendell update --yes", "Non-interactive (accept downgrade prompts)"],
        ["sendell update wizard", "Interactive update wizard"],
        ["sendell --update", "Shorthand for sendell update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run sendell update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.sendell.bot/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["sendell update status", "Show channel + version status."],
          ["sendell update status --json", "JSON output."],
          ["sendell update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.sendell.bot/cli/update")}`,
    )
    .action(async (opts) => {
      try {
