...
function resolveProfileStateDir(profile: string, homedir: () => string): string {
  ...
  return path.join(homedir(), `.sendell${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.SENDELL_PROFILE = profile;

  const stateDir = env.SENDELL_STATE_DIR?.trim() || resolveProfileStateDir(profile, homedir);
  if (!env.SENDELL_STATE_DIR?.trim()) env.SENDELL_STATE_DIR = stateDir;

  if (!env.SENDELL_CONFIG_PATH?.trim()) {
    env.SENDELL_CONFIG_PATH = path.join(stateDir, "sendell.json");
  }

  if (profile === "dev" && !env.SENDELL_GATEWAY_PORT?.trim()) {
    env.SENDELL_GATEWAY_PORT = "19001";
  }
}
