...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "sendell-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      SENDELL_NO_RESPAWN: "1",
      SENDELL_STATE_DIR: stateDir,
      SENDELL_SKIP_CHANNELS: "1",
      SENDELL_SKIP_GMAIL_WATCHER: "1",
      SENDELL_SKIP_CRON: "1",
      SENDELL_SKIP_BROWSER_CONTROL_SERVER: "1",
      SENDELL_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "sendell-entry-bootstrap.mjs");
