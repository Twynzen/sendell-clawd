...
import { STATE_DIR_SENDELL } from "../config/paths.js";
...
function installedExtensionRootDir() {
  return path.join(STATE_DIR_SENDELL, "browser", "chrome-extension");
}
...
export async function installChromeExtension(opts?: {
  ...
  if (!hasManifest(src)) {
    throw new Error("Bundled Chrome extension is missing. Reinstall Sendell and try again.");
  }

  const stateDir = opts?.stateDir ?? STATE_DIR_SENDELL;
      ...
      defaultRuntime.error(
        info(
          [
            ...
            `- Pin “Sendell Browser Relay”, then click it on the tab (badge shows ON)`,
            "",
            `${theme.muted("Docs:")} ${formatDocsLink("/tools/chrome-extension", "docs.sendell.bot/tools/chrome-extension")}`,
          ].join("\n"),
        ),
      );
    });
    ...
    .action(async (_opts, cmd) => {
      ...
      if (!hasManifest(dir)) {
        defaultRuntime.error(
          danger(
            [
              `Chrome extension is not installed. Run: "${formatCliCommand("sendell browser extension install")}"`,
              `Docs: ${formatDocsLink("/tools/chrome-extension", "docs.sendell.bot/tools/chrome-extension")}`,
            ].join("\n"),
          ),
        );
        defaultRuntime.exit(1);
