...
async function ensureSendellExtensions(manifest: PackageManifest) {
  const extensions = manifest.sendell?.extensions;
  if (!Array.isArray(extensions)) {
    throw new Error("package.json missing sendell.extensions");
  }
  const list = extensions.map((e) => (typeof e === "string" ? e.trim() : "")).filter(Boolean);
  if (list.length === 0) {
    throw new Error("package.json sendell.extensions is empty");
  }
  return list;
}
...
async function installPluginFromPackageDir(params: {
  ...
  try {
    extensions = await ensureSendellExtensions(manifest);
  } catch (err) {
...
export async function installPluginFromArchive(params: {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "sendell-plugin-"));
