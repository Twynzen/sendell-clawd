...
import { loadSendellPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.SENDELL_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `sendell-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "sendell.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.SENDELL_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadSendellPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.SENDELL_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadSendellPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadSendellPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
