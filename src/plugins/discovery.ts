...
import type { SendellPackageManifest, PackageManifest } from "./manifest.js";
...
function resolvePackageExtensions(manifest: PackageManifest): string[] {
  const raw = manifest.sendell?.extensions;
...
function deriveIdHint(params: {
  ...
  // package is scoped (example: @sendell/voice-call -> voice-call).
...
function addCandidate(params: {
  ...
  params.candidates.push({
    ...
    packageSendell: manifest?.sendell,
  });
}
...
export function discoverSendellPlugins(params: {
  workspaceDir?: string;
  extraPaths?: string[];
  ...
  if (workspaceDir) {
    ...
    const workspaceExt = path.join(workspaceRoot, ".sendell", "extensions");
    discoverInDirectory({
      dir: workspaceExt,
      origin: "workspace",
