...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: SendellPluginDefinition;
  register?: SendellPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as SendellPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as SendellPluginDefinition;
...
export function loadSendellPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias ? { alias: { "sendell/plugin-sdk": pluginSdkAlias } } : {}),
  });
