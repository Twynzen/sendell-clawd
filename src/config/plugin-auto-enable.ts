import type { SendellConfig } from "./config.js";
import {
  getChatChannelMeta,
  listChatChannels,
...
function isTelegramConfigured(cfg: SendellConfig, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: SendellConfig, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: SendellConfig, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: SendellConfig): boolean {
...
function isIMessageConfigured(cfg: SendellConfig): boolean {
...
function isWhatsAppConfigured(cfg: SendellConfig): boolean {
...
function isGenericChannelConfigured(cfg: SendellConfig, channelId: string): boolean {
...
function collectModelRefs(cfg: SendellConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: SendellConfig, providerId: string): boolean {
...
function isPluginExplicitlyDisabled(cfg: SendellConfig, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: SendellConfig, pluginId: string): boolean {
...
function ensureAllowlisted(cfg: SendellConfig, pluginId: string): SendellConfig {
...
function enablePluginEntry(cfg: SendellConfig, pluginId: string): SendellConfig {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
