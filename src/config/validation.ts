...
import type { SendellConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: SendellConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: SendellConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = SendellSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as SendellConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as SendellConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as SendellConfig)),
    ),
  };
}
