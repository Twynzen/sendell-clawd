...
type AgentConfig = Partial<NonNullable<NonNullable<SendellConfig["agents"]>["defaults"]>>;
...
const readUsageFromSessionLog = (
  ...
  | undefined => {
  // Transcripts are stored at the session file path (fallback: ~/.sendell/sessions/<SessionId>.jsonl)
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `👻 Sendell ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: SendellConfig): string {
  const options = [
    "/think <level>",
    "/verbose on|full|off",
    "/reasoning on|off",
