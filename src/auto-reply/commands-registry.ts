import type { SendellConfig } from "../config/types.js";
...
export function isCommandEnabled(cfg: SendellConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: SendellConfig): {
  provider: string;
  model: string;
} {
...
export function getCommandDetection(_cfg?: SendellConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: SendellConfig) {
