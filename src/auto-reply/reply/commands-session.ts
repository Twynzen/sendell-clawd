...
import { scheduleGatewaySigusr1Restart, triggerSendellRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting sendell in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerSendellRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting sendell via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
