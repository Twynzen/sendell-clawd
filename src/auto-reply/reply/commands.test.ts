...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "sendell-commands-"));
...
function buildParams(commandBody: string, cfg: SendellConfig, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,
