...
function resolveTaskName(env: Record<string, string | undefined>): string {
  const override = env.SENDELL_WINDOWS_TASK_NAME?.trim();
  if (override) return override;
  return resolveGatewayWindowsTaskName(env.SENDELL_PROFILE);
}

export function resolveTaskScriptPath(env: Record<string, string | undefined>): string {
  const override = env.SENDELL_TASK_SCRIPT?.trim();
  if (override) return override;
  const scriptName = env.SENDELL_TASK_SCRIPT_NAME?.trim() || "gateway.cmd";
...
export async function installScheduledTask({
  ...
  const taskDescription =
    ...
    formatGatewayServiceDescription({
      profile: env.SENDELL_PROFILE,
      version: environment?.SENDELL_SERVICE_VERSION ?? env.SENDELL_SERVICE_VERSION,
    });
