...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "SENDELL_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "sendell-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.SENDELL_STATE_DIR = path.join(home, ".sendell");
    ...
    await fs.mkdir(path.join(home, ".sendell"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.sendell${path.sep}media`);
