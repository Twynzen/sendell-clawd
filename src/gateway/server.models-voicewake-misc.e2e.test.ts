...
describe("gateway server models + voicewake", () => {
  const setTempHome = (homeDir: string) => {
    ...
    process.env.SENDELL_STATE_DIR = path.join(homeDir, ".sendell");
    ...
    return () => {
      ...
      if (prevStateDir === undefined) {
        ...
        process.env.SENDELL_STATE_DIR = prevStateDir;
      }
  ...
  test(
    ...
    { timeout: 60_000 },
    async () => {
      const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "sendell-home-"));
      const restoreHome = setTempHome(homeDir);

      const initial = await rpcReq<{ triggers: string[] }>(ws, "voicewake.get");
      expect(initial.ok).toBe(true);
      expect(initial.payload?.triggers).toEqual(["sendell", "claude", "computer"]);
      ...
      const onDisk = JSON.parse(
        await fs.readFile(path.join(homeDir, ".sendell", "settings", "voicewake.json"), "utf8"),
...
describe("gateway server misc", () => {
  test("hello-ok advertises the gateway port for canvas host", async () => {
    ...
    const prevCanvasPort = process.env.SENDELL_CANVAS_HOST_PORT;
    process.env.SENDELL_GATEWAY_TOKEN = "secret";
    testTailnetIPv4.value = "100.64.0.1";
    testState.gatewayBind = "lan";
    const canvasPort = await getFreePort();
    testState.canvasHostPort = canvasPort;
    process.env.SENDELL_CANVAS_HOST_PORT = String(canvasPort);
    ...
    if (prevToken === undefined) {
      ...
      process.env.SENDELL_GATEWAY_TOKEN = prevToken;
    }
    if (prevCanvasPort === undefined) {
      delete process.env.SENDELL_CANVAS_HOST_PORT;
    } else {
      process.env.SENDELL_CANVAS_HOST_PORT = prevCanvasPort;
    }
  });
  ...
  test("auto-enables configured channel plugins on startup", async () => {
    const configPath = process.env.SENDELL_CONFIG_PATH;
    if (!configPath) throw new Error("Missing SENDELL_CONFIG_PATH");
