...
export function resolveHooksConfig(cfg: SendellConfig): HooksConfigResolved | null {
...
export function extractHookToken(req: IncomingMessage, url: URL): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-sendell-token"] === "string" ? req.headers["x-sendell-token"].trim() : "";
