...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `sendell-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.SENDELL_CONFIG_PATH = path.join(root, "sendell.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "sendell.json");
  ...
  return {
    ...actual,
    get CONFIG_PATH_SENDELL() {
      return resolveConfigPath();
    },
    get STATE_DIR_SENDELL() {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "sendell-gateway-test"),
        ...fileDefaults,
