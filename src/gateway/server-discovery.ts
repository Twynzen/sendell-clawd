...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (!trimmed) return "Sendell";
  if (/sendell/i.test(trimmed)) return trimmed;
  return `${trimmed} (Sendell)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.SENDELL_CLI_PATH?.trim();
  ...
  const siblingCli = path.join(execDir, "sendell");
  ...
  const binCli = path.join(cwd, "bin", "sendell.js");
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.SENDELL_TAILNET_DNS?.trim();
