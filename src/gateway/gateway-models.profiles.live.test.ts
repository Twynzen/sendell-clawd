...
import { resolveSendellAgentDir } from "../agents/agent-paths.js";
...
import { ensureSendellModelsJson } from "../agents/models-config.js";
import { loadConfig } from "../config/config.js";
import type { SendellConfig, ModelProviderConfig } from "../config/types.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.SENDELL_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.SENDELL_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.SENDELL_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<SendellConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.SENDELL_AGENT_DIR,
  ...
  const hostAgentDir = resolveSendellAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "sendell-live-state-"));
  ...
  process.env.SENDELL_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.sendell-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveSendellAgentDir();
  const sanitizedCfg: SendellConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "Sendell live tool probe (local, safe): " +
    ...
    process.env.SENDELL_CONFIG_PATH = previous.configPath;
    process.env.SENDELL_GATEWAY_TOKEN = previous.token;
    process.env.SENDELL_SKIP_CHANNELS = previous.skipChannels;
    process.env.SENDELL_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.SENDELL_SKIP_CRON = previous.skipCron;
    process.env.SENDELL_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.SENDELL_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.SENDELL_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureSendellModelsJson(cfg);
      ...
      const rawModels = process.env.SENDELL_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.sendell-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
