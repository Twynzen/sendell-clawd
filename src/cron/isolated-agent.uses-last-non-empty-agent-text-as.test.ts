...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "sendell-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".sendell", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<SendellConfig> = {},
): SendellConfig {
  const base: SendellConfig = {
    agents: {
      defaults: {
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".sendell", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
