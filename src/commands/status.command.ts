...
export async function statusCommand(
  ...
  runtime.log(theme.heading("Sendell status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("sendell security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("sendell security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.sendell.bot/faq");
  runtime.log("Troubleshooting: https://docs.sendell.bot/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("sendell status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("sendell logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("sendell status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("sendell gateway probe")}`);
  }
}
