...
function resolveConfiguredModelRaw(cfg: SendellConfig): string {
  const raw = cfg.agents?.defaults?.model as { primary?: string } | string | undefined;
  if (typeof raw === "string") return raw.trim();
  return raw?.primary?.trim() ?? "";
}

function resolveConfiguredModelKeys(cfg: SendellConfig): string[] {
...
export function applyPrimaryModel(cfg: SendellConfig, model: string): SendellConfig {
...
export function applyModelAllowlist(cfg: SendellConfig, models: string[]): SendellConfig {
  const defaults = cfg.agents?.defaults;
