...
function resolveDefaultAccountId(cfg: SendellConfig, provider: ChannelId): string {
  const plugin = getChannelPlugin(provider);
  if (!plugin) return DEFAULT_ACCOUNT_ID;
  return resolveChannelDefaultAccountId({ plugin, cfg });
}

function shouldShowProviderEntry(entry: ProviderAccountStatus, cfg: SendellConfig): boolean {
...
export function summarizeBindings(cfg: SendellConfig, bindings: AgentBinding[]): string[] {
