...
function stripUnknownConfigKeys(config: SendellConfig): {
  config: SendellConfig;
  removed: string[];
} {
  const parsed = SendellSchema.safeParse(config);
  if (parsed.success) {
    return { config, removed: [] };
  }

  const next = structuredClone(config) as SendellConfig;
...
function noteOpencodeProviderOverrides(cfg: SendellConfig) {
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  let cfg: SendellConfig = baseCfg;
  let candidate = structuredClone(baseCfg) as SendellConfig;
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("sendell doctor --fix")}" to apply legacy migrations.`,
      );
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("sendell doctor --fix")}" to apply these changes.`);
    }
  }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "sendell doctor --fix" to remove these keys.');
    }
  }
  ...
  return { cfg, path: snapshot.path ?? CONFIG_PATH_SENDELL, shouldWriteConfig };
}
