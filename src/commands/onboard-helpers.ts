...
export function summarizeExistingConfig(config: SendellConfig): string {
  const rows: string[] = [];
...
export function formatControlUiSshHint(params: {
  ...
  return [
    ...
    authedUrl,
    "Docs:",
    "https://docs.sendell.bot/gateway/remote",
    "https://docs.sendell.bot/web/control-ui",
  ]
    .filter(Boolean)
    .join("\n");
}
...
export async function handleReset(scope: ResetScope, workspaceDir: string, runtime: RuntimeEnv) {
  await moveToTrash(CONFIG_PATH_SENDELL, runtime);
