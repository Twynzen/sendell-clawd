...
beforeEach(() => {
  ...
  resolveSendellPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.SENDELL_STATE_DIR;
  originalUpdateInProgress = process.env.SENDELL_UPDATE_IN_PROGRESS;
  process.env.SENDELL_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "sendell-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.SENDELL_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.SENDELL_UPDATE_IN_PROGRESS;
  } else {
    process.env.SENDELL_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveSendellPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadSendellPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    CONFIG_PATH_SENDELL: "/tmp/sendell.json",
    createConfigIO,
...
vi.mock("../infra/sendell-root.js", () => ({
  resolveSendellPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "sendell-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.SENDELL_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when SENDELL_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/sendell.json",
      exists: true,
      raw: "{}",
    ...
    process.env.SENDELL_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
