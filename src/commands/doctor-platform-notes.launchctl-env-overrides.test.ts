...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "SENDELL_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("SENDELL_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv SENDELL_GATEWAY_TOKEN");
    expect(message).not.toContain("SENDELL_GATEWAY_PASSWORD");
  });
