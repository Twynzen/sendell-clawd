...
import { readConfigFileSnapshot, type SendellConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: SendellConfig, model: string): SendellConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("sendell plugins install")}\`.`,
    );
  }
