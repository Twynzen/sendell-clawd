...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveSendellAgentDir: vi.fn().mockReturnValue("/tmp/sendell-agent"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveSendellAgentDir: mocks.resolveSendellAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.auth.storePath).toBe("/tmp/sendell-agent/auth-profiles.json");
