...
import { type SendellConfig, CONFIG_PATH_SENDELL, writeConfigFile } from "../config/config.js";
...
async function readConfigFileRaw(): Promise<{
  exists: boolean;
  parsed: SendellConfig;
}> {
  try {
    const raw = await fs.readFile(CONFIG_PATH_SENDELL, "utf-8");
    const parsed = JSON5.parse(raw);
    if (parsed && typeof parsed === "object") {
      return { exists: true, parsed: parsed as SendellConfig };
    }
