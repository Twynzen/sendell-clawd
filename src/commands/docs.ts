...
const SEARCH_TOOL = "https://docs.sendell.bot/mcp.SearchSendell";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.sendell.bot");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('sendell docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.sendell.bot/");
      runtime.log(`Search: ${formatCliCommand('sendell docs "your query"')}`);
    }
    return;
  }
