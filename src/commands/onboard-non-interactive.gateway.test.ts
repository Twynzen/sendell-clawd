...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "sendell-onboard-"));
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "sendell");
  ...
  it("auto-enables token auth when binding LAN and persists the token", async () => {
    ...
    process.env.SENDELL_STATE_DIR = stateDir;
    process.env.SENDELL_CONFIG_PATH = path.join(stateDir, "sendell.json");
