...
vi.mock("../infra/sendell-root.js", () => ({
  resolveSendellPackageRoot: vi.fn().mockResolvedValue("/tmp/sendell"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/sendell/pnpm-lock.yaml",
      markerPath: "/tmp/sendell/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/com.sendell.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/com.sendell.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("Sendell status"))).toBe(true);
    ...
    expect(logs.some((l) => l.includes("sendell status --all"))).toBe(true);
  });

  it("shows gateway auth when reachable", async () => {
    const prevToken = process.env.SENDELL_GATEWAY_TOKEN;
    process.env.SENDELL_GATEWAY_TOKEN = "abcd1234";
    try {
