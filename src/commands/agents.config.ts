...
export function listAgentEntries(cfg: SendellConfig): AgentEntry[] {
...
function resolveAgentName(cfg: SendellConfig, agentId: string) {
...
function resolveAgentModel(cfg: SendellConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: SendellConfig): AgentSummary[] {
