...
const requireAgentDir = () => {
  const agentDir = process.env.SENDELL_AGENT_DIR;
  if (!agentDir) throw new Error("SENDELL_AGENT_DIR not set");
  return agentDir;
};
...
describe("writeOAuthCredentials", () => {
  ...
  afterEach(async () => {
    ...
    delete process.env.SENDELL_OAUTH_DIR;
  });

  it("writes auth-profiles.json under SENDELL_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "sendell-oauth-"));
    process.env.SENDELL_STATE_DIR = tempStateDir;
    process.env.SENDELL_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.SENDELL_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to SENDELL_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "sendell-minimax-"));
    process.env.SENDELL_STATE_DIR = tempStateDir;
    process.env.SENDELL_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.SENDELL_AGENT_DIR;
