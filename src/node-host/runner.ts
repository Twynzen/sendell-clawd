...
const execHostEnforced = process.env.SENDELL_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.SENDELL_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureSendellCliOnPath({ pathEnv: process.env.PATH ?? "" });
...
async function handleInvoke(
  ...
  if (command === "browser.proxy") {
    try {
      ...
      const token =
        process.env.SENDELL_BROWSER_CONTROL_TOKEN?.trim() || resolved.controlToken?.trim();
      if (token) {
