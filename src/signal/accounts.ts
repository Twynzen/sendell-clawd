...
export function listSignalAccountIds(cfg: SendellConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSignalAccountId(cfg: SendellConfig): string {
...
function mergeSignalAccountConfig(cfg: SendellConfig, accountId: string): SignalAccountConfig {
...
export function listEnabledSignalAccounts(cfg: SendellConfig): ResolvedSignalAccount[] {
