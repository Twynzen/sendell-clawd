...
function candidateBinDirs(opts: EnsureSendellPathOpts): string[] {
  ...
  // Bundled macOS app: `sendell` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingSendell = path.join(execDir, "sendell");
    if (isExecutable(siblingSendell)) candidates.push(execDir);
  } catch {
    // ignore
  }

  // Project-local installs (best effort): if a `node_modules/.bin/sendell` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "sendell"))) candidates.push(localBinDir);
...
/**
 * Best-effort PATH bootstrap so skills that require the `sendell` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureSendellCliOnPath(opts: EnsureSendellPathOpts = {}) {
  if (isTruthyEnvValue(process.env.SENDELL_PATH_BOOTSTRAPPED)) return;
  process.env.SENDELL_PATH_BOOTSTRAPPED = "1";
