...
export function triggerSendellRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.SENDELL_SYSTEMD_UNIT,
        process.env.SENDELL_PROFILE,
      );
  ...
  const label =
    process.env.SENDELL_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.SENDELL_PROFILE);
