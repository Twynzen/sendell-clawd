...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: SendellConfig | null = null;

  constructor(private readonly ttlMs: number) {}

  get(key: string, cfg: SendellConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: SendellConfig): void {
    this.resetIfConfigChanged(cfg);
  ...
  clear(cfg?: SendellConfig): void {
    this.cache.clear();
    if (cfg) this.lastConfigRef = cfg;
  }

  private resetIfConfigChanged(cfg: SendellConfig): void {
