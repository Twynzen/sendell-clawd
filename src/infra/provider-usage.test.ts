...
describe("provider usage loading", () => {
  ...
  it("discovers Claude usage from token auth profiles", async () => {
    await withTempHome(
      ...
      {
        env: {
          SENDELL_STATE_DIR: (home) => path.join(home, ".sendell"),
        },
        prefix: "sendell-provider-usage-",
      },
    );
  });
  ...
  it("prefers claude-cli token for Anthropic usage snapshots", async () => {
    await withTempHome(
      async () => {
        const stateDir = process.env.SENDELL_STATE_DIR;
        if (!stateDir) throw new Error("Missing SENDELL_STATE_DIR");
