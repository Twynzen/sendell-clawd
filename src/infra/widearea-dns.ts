...
export const WIDE_AREA_DISCOVERY_DOMAIN = "sendell.internal.";
export const WIDE_AREA_ZONE_FILENAME = "sendell.internal.db";
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*sendell-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "sendell";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "sendell");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "sendell-gw");
  ...
  records.push(`_sendell-gw._tcp IN PTR ${instanceLabel}._sendell-gw._tcp`);
  records.push(`${instanceLabel}._sendell-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._sendell-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; sendell-content-hash: ${contentHash}\n${contentBody}`;
}
