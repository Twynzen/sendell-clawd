...
function listWorkspaceDirs(cfg: SendellConfig): string[] {
...
function collectRequiredBins(entries: SkillEntry[], targetPlatform: string): string[] {
  ...
  for (const entry of entries) {
    const os = entry.sendell?.os ?? [];
    if (os.length > 0 && !os.includes(targetPlatform)) continue;
    const required = entry.sendell?.requires?.bins ?? [];
    const anyBins = entry.sendell?.requires?.anyBins ?? [];
...
export async function refreshRemoteBinsForConnectedNodes(cfg: SendellConfig) {
  if (!remoteRegistry) return;
