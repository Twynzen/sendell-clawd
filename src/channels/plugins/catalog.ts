...
import { discoverSendellPlugins } from "../../plugins/discovery.js";
import type { PluginOrigin } from "../../plugins/types.js";
import type { SendellPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
  sendell?: SendellPackageManifest;
};
...
const ENV_CATALOG_PATHS = ["SENDELL_PLUGIN_CATALOG_PATHS", "SENDELL_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<SendellPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: SendellPackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageSendell?: SendellPackageManifest;
}): ChannelPluginCatalogEntry | null {
  const manifest = candidate.packageSendell;
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverSendellPlugins({ workspaceDir: options.workspaceDir });
