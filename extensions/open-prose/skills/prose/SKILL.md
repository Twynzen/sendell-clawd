...
metadata: {"sendell":{"emoji":"🪶","homepage":"https://www.prose.md"}}
---
...
- **Task tool** in the upstream spec == Sendell `sessions_spawn`
- **File I/O** == Sendell `read`/`write`
- **Remote fetch** == Sendell `web_fetch` (or `exec` with curl when POST is required)
