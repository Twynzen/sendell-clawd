import type { SendellConfig } from "sendell/plugin-sdk";
...
export function listZaloAccountIds(cfg: SendellConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultZaloAccountId(cfg: SendellConfig): string {
...
function mergeZaloAccountConfig(cfg: SendellConfig, accountId: string): ZaloAccountConfig {
...
export function listEnabledZaloAccounts(cfg: SendellConfig): ResolvedZaloAccount[] {
