...
Sendell extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
sendell channels login --channel zalouser
...
```bash
sendell message send --channel zalouser --target <threadId> --message "Hello from Sendell!"
```
...
```bash
sendell channels login --channel zalouser              # Login via QR
sendell channels login --channel zalouser --account work
sendell channels status --probe
sendell channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
sendell directory self --channel zalouser
sendell directory peers list --channel zalouser --query "name"
sendell directory groups list --channel zalouser --query "work"
sendell directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
sendell message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
sendell message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `sendell channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
sendell channels login --channel zalouser --account work
sendell message send --channel zalouser --account work --target <id> --message "Hello"
