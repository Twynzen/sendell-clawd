...
// NOTE: This extension is intended to be bundled with Sendell.
// When running from source (tests/dev), Sendell internals live under src/.
...
export function createLlmTaskTool(api: SendellPluginApi) {
  return {
    name: "llm-task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via sendell.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "sendell-llm-task-"));
