---
description: Update Sendell from upstream when branch has diverged (ahead/behind)
---

# Sendell Upstream Sync Workflow
...
```bash
# Kill running app
pkill -x "Sendell" || true

# Move old version
mv /Applications/Sendell.app /tmp/Sendell-backup.app

# Install new build
cp -R dist/Sendell.app /Applications/

# Launch
open /Applications/Sendell.app
```
...
ps aux | grep -E "(sendell|gateway)" | grep -v grep

# Test agent functionality by sending a verification message
pnpm sendell agent --message "Verification: macOS app rebuild successful - agent is responding." --session-id YOUR_TELEGRAM_SESSION_ID
...
# Update sendell.json with fallback chains
...
pnpm sendell agent --message "Verification: Upstream sync and macOS rebuild completed successfully." --session-id YOUR_TELEGRAM_SESSION_ID || echo "Warning: Agent test failed - check Telegram for verification message"
