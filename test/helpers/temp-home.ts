...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("SENDELL_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.SENDELL_STATE_DIR = path.join(base, ".sendell");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "sendell-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".sendell", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
