...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `sendell-e2e-${name}-`));
  const configDir = path.join(homeDir, ".sendell");
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        SENDELL_GATEWAY_TOKEN: inst.gatewayToken,
        SENDELL_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          SENDELL_GATEWAY_PORT: String(gwA.port),
          SENDELL_GATEWAY_TOKEN: gwA.gatewayToken,
          SENDELL_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          SENDELL_GATEWAY_PORT: String(gwB.port),
          SENDELL_GATEWAY_TOKEN: gwB.gatewayToken,
          SENDELL_GATEWAY_PASSWORD: "",
        }),
