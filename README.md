# 👻 Sendell — Personal AI Companion

<p align="center">
  <em>A companion in the exploration of existence.</em>
</p>
...
> Based on [Clawdbot](https://github.com/clawdbot/clawdbot) by [@steipete](https://github.com/steipete). Original docs: [docs.clawd.bot](https://docs.clawd.bot)

**Sendell** is a *personal AI assistant* you run on your own devices.
It answers you on the channels you already use (WhatsApp, Telegram, Slack, Discord, Google Chat, Signal, iMessage, Microsoft Teams, WebChat), plus extension channels like BlueBubbles, Matrix, Zalo, and Zalo Personal. It can speak and listen on macOS/iOS/Android, and can render a live Canvas you control. The Gateway is just the control plane — the product is the assistant.

If you want a personal, single-user assistant that feels local, fast, and always-on, this is it.

For original project documentation, see [docs.clawd.bot](https://docs.clawd.bot).

Preferred setup: run the onboarding wizard (`sendell onboard`). It walks through gateway, workspace, channels, and skills. The CLI wizard is the recommended path and works on **macOS, Linux, and Windows (via WSL2; strongly recommended)**.
Works with npm, pnpm, or bun.
New install? Start here: [Getting started](https://docs.sendell.bot/start/getting-started)

**Subscriptions (OAuth):**
- **[Anthropic](https://www.anthropic.com/)** (Claude Pro/Max)
- **[OpenAI](https://openai.com/)** (ChatGPT/Codex)

Model note: while any model is supported, I strongly recommend **Anthropic Pro/Max (100/200) + Opus 4.5** for long‑context strength and better prompt‑injection resistance. See [Onboarding](https://docs.sendell.bot/start/onboarding).

## Models (selection + auth)

- Models config + CLI: [Models](https://docs.sendell.bot/concepts/models)
- Auth profile rotation (OAuth vs API keys) + fallbacks: [Model failover](https://docs.sendell.bot/concepts/model-failover)
...
Full beginner guide (auth, pairing, channels): [Getting started](https://docs.sendell.bot/start/getting-started)

```bash
sendell onboard --install-daemon

sendell gateway --port 18789 --verbose

# Send a message
sendell message send --to +1234567890 --message "Hello from Sendell"

# Talk to the assistant (optionally deliver back to any connected channel: WhatsApp/Telegram/Slack/Discord/Google Chat/Signal/iMessage/BlueBubbles/Microsoft Teams/Matrix/Zalo/Zalo Personal/WebChat)
sendell agent --message "Ship checklist" --thinking high
```

Upgrading? [Updating guide](https://docs.sendell.bot/install/updating) (and run `sendell doctor`).
...
Switch channels (git + npm): `sendell update --channel stable|beta|dev`.
Details: [Development channels](https://docs.sendell.bot/install/development-channels).
...
pnpm sendell onboard --install-daemon

# Dev loop (auto-reload on TS changes)
pnpm gateway:watch
```

Note: `pnpm sendell ...` runs TypeScript directly (via `tsx`). `pnpm build` produces `dist/` for running via Node / the packaged `sendell` binary.

## Security defaults (DM access)

Sendell connects to real messaging surfaces. Treat inbound DMs as **untrusted input**.

Full security guide: [Security](https://docs.sendell.bot/gateway/security)

Default behavior on Telegram/WhatsApp/Signal/iMessage/Microsoft Teams/Discord/Google Chat/Slack:
- **DM pairing** (`dmPolicy="pairing"` / `channels.discord.dm.policy="pairing"` / `channels.slack.dm.policy="pairing"`): unknown senders receive a short pairing code and the bot does not process their message.
- Approve with: `sendell pairing approve <channel> <code>` (then the sender is added to a local allowlist store).
- Public inbound DMs require an explicit opt-in: set `dmPolicy="open"` and include `"*"` in the channel allowlist (`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`).

Run `sendell doctor` to surface risky/misconfigured DM policies.

## Highlights

- **[Local-first Gateway](https://docs.sendell.bot/gateway)** — single control plane for sessions, channels, tools, and events.
- **[Multi-channel inbox](https://docs.sendell.bot/channels)** — WhatsApp, Telegram, Slack, Discord, Google Chat, Signal, iMessage, BlueBubbles, Microsoft Teams, Matrix, Zalo, Zalo Personal, WebChat, macOS, iOS/Android.
- **[Multi-agent routing](https://docs.sendell.bot/gateway/configuration)** — route inbound channels/accounts/peers to isolated agents (workspaces + per-agent sessions).
- **[Voice Wake](https://docs.sendell.bot/nodes/voicewake) + [Talk Mode](https://docs.sendell.bot/nodes/talk)** — always-on speech for macOS/iOS/Android with ElevenLabs.
- **[Live Canvas](https://docs.sendell.bot/platforms/mac/canvas)** — agent-driven visual workspace with [A2UI](https://docs.sendell.bot/platforms/mac/canvas#canvas-a2ui).
- **[First-class tools](https://docs.sendell.bot/tools)** — browser, canvas, nodes, cron, sessions, and Discord/Slack actions.
- **[Companion apps](https://docs.sendell.bot/platforms/macos)** — macOS menu bar app + iOS/Android [nodes](https://docs.sendell.bot/nodes).
- **[Onboarding](https://docs.sendell.bot/start/wizard) + [skills](https://docs.sendell.bot/tools/skills)** — wizard-driven setup with bundled/managed/workspace skills.

## Star History

[![Star History Chart](https://api.star-history.com/svg?repos=sendell/sendell&type=date&legend=top-left)](https://www.star-history.com/#sendell/sendell&type=date&legend=top-left)

## Everything we built so far

### Core platform
- [Gateway WS control plane](https://docs.sendell.bot/gateway) with sessions, presence, config, cron, webhooks, [Control UI](https://docs.sendell.bot/web), and [Canvas host](https://docs.sendell.bot/platforms/mac/canvas#canvas-a2ui).
- [CLI surface](https://docs.sendell.bot/tools/agent-send): gateway, agent, send, [wizard](https://docs.sendell.bot/start/wizard), and [doctor](https://docs.sendell.bot/gateway/doctor).
- [Pi agent runtime](https://docs.sendell.bot/concepts/agent) in RPC mode with tool streaming and block streaming.
- [Session model](https://docs.sendell.bot/concepts/session): `main` for direct chats, group isolation, activation modes, queue modes, reply-back. Group rules: [Groups](https://docs.sendell.bot/concepts/groups).
- [Media pipeline](https://docs.sendell.bot/nodes/images): images/audio/video, transcription hooks, size caps, temp file lifecycle. Audio details: [Audio](https://docs.sendell.bot/nodes/audio).

### Channels
- [Channels](https://docs.sendell.bot/channels): [WhatsApp](https://docs.sendell.bot/channels/whatsapp) (Baileys), [Telegram](https://docs.sendell.bot/channels/telegram) (grammY), [Slack](https://docs.sendell.bot/channels/slack) (Bolt), [Discord](https://docs.sendell.bot/channels/discord) (discord.js), [Google Chat](https://docs.sendell.bot/channels/googlechat) (Chat API), [Signal](https://docs.sendell.bot/channels/signal) (signal-cli), [iMessage](https://docs.sendell.bot/channels/imessage) (imsg), [BlueBubbles](https://docs.sendell.bot/channels/bluebubbles) (extension), [Microsoft Teams](https://docs.sendell.bot/channels/msteams) (extension), [Matrix](https://docs.sendell.bot/channels/matrix) (extension), [Zalo](https://docs.sendell.bot/channels/zalo) (extension), [Zalo Personal](https://docs.sendell.bot/channels/zalouser) (extension), [WebChat](https://docs.sendell.bot/web/webchat).
- [Group routing](https://docs.sendell.bot/concepts/group-messages): mention gating, reply tags, per-channel chunking and routing. Channel rules: [Channels](https://docs.sendell.bot/channels).

### Apps + nodes
- [macOS app](https://docs.sendell.bot/platforms/macos): menu bar control plane, [Voice Wake](https://docs.sendell.bot/nodes/voicewake)/PTT, [Talk Mode](https://docs.sendell.bot/nodes/talk) overlay, [WebChat](https://docs.sendell.bot/web/webchat), debug tools, [remote gateway](https://docs.sendell.bot/gateway/remote) control.
- [iOS node](https://docs.sendell.bot/platforms/ios): [Canvas](https://docs.sendell.bot/platforms/mac/canvas), [Voice Wake](https://docs.sendell.bot/nodes/voicewake), [Talk Mode](https://docs.sendell.bot/nodes/talk), camera, screen recording, Bonjour pairing.
- [Android node](https://docs.sendell.bot/platforms/android): [Canvas](https://docs.sendell.bot/platforms/mac/canvas), [Talk Mode](https://docs.sendell.bot/nodes/talk), camera, screen recording, optional SMS.
- [macOS node mode](https://docs.sendell.bot/nodes): system.run/notify + canvas/camera exposure.

### Tools + automation
- [Browser control](https://docs.sendell.bot/tools/browser): dedicated sendell Chrome/Chromium, snapshots, actions, uploads, profiles.
- [Canvas](https://docs.sendell.bot/platforms/mac/canvas): [A2UI](https://docs.sendell.bot/platforms/mac/canvas#canvas-a2ui) push/reset, eval, snapshot.
- [Nodes](https://docs.sendell.bot/nodes): camera snap/clip, screen record, [location.get](https://docs.sendell.bot/nodes/location-command), notifications.
- [Cron + wakeups](https://docs.sendell.bot/automation/cron-jobs); [webhooks](https://docs.sendell.bot/automation/webhook); [Gmail Pub/Sub](https://docs.sendell.bot/automation/gmail-pubsub).
- [Skills platform](https://docs.sendell.bot/tools/skills): bundled, managed, and workspace skills with install gating + UI.

### Runtime + safety
- [Channel routing](https://docs.sendell.bot/concepts/channel-routing), [retry policy](https://docs.sendell.bot/concepts/retry), and [streaming/chunking](https://docs.sendell.bot/concepts/streaming).
- [Presence](https://docs.sendell.bot/concepts/presence), [typing indicators](https://docs.sendell.bot/concepts/typing-indicators), and [usage tracking](https://docs.sendell.bot/concepts/usage-tracking).
- [Models](https://docs.sendell.bot/concepts/models), [model failover](https://docs.sendell.bot/concepts/model-failover), and [session pruning](https://docs.sendell.bot/concepts/session-pruning).
- [Security](https://docs.sendell.bot/gateway/security) and [troubleshooting](https://docs.sendell.bot/channels/troubleshooting).

### Ops + packaging
- [Control UI](https://docs.sendell.bot/web) + [WebChat](https://docs.sendell.bot/web/webchat) served directly from the Gateway.
- [Tailscale Serve/Funnel](https://docs.sendell.bot/gateway/tailscale) or [SSH tunnels](https://docs.sendell.bot/gateway/remote) with token/password auth.
- [Nix mode](https://docs.sendell.bot/install/nix) for declarative config; [Docker](https://docs.sendell.bot/install/docker)-based installs.
- [Doctor](https://docs.sendell.bot/gateway/doctor) migrations, [logging](https://docs.sendell.bot/logging).
...
- **[Gateway WebSocket network](https://docs.sendell.bot/concepts/architecture)** — single WS control plane for clients, tools, and events (plus ops: [Gateway runbook](https://docs.sendell.bot/gateway)).
- **[Tailscale exposure](https://docs.sendell.bot/gateway/tailscale)** — Serve/Funnel for the Gateway dashboard + WS (remote access: [Remote](https://docs.sendell.bot/gateway/remote)).
- **[Browser control](https://docs.sendell.bot/tools/browser)** — sendell‑managed Chrome/Chromium with CDP control.
- **[Canvas + A2UI](https://docs.sendell.bot/platforms/mac/canvas)** — agent‑driven visual workspace (A2UI host: [Canvas/A2UI](https://docs.sendell.bot/platforms/mac/canvas#canvas-a2ui)).
- **[Voice Wake](https://docs.sendell.bot/nodes/voicewake) + [Talk Mode](https://docs.sendell.bot/nodes/talk)** — always‑on speech and continuous conversation.
- **[Nodes](https://docs.sendell.bot/nodes)** — Canvas, camera snap/clip, screen record, `location.get`, notifications, plus macOS‑only `system.run`/`system.notify`.

## Tailscale access (Gateway dashboard)

Sendell can auto-configure Tailscale **Serve** (tailnet-only) or **Funnel** (public) while the Gateway stays bound to loopback. Configure `gateway.tailscale.mode`:
...
Notes:
- `gateway.bind` must stay `loopback` when Serve/Funnel is enabled (Sendell enforces this).
- Serve can be forced to require a password by setting `gateway.auth.mode: "password"` or `gateway.auth.allowTailscale: false`.
- Funnel refuses to start unless `gateway.auth.mode: "password"` is set.
- Optional: `gateway.tailscale.resetOnExit` to undo Serve/Funnel on shutdown.

Details: [Tailscale guide](https://docs.sendell.bot/gateway/tailscale) · [Web surfaces](https://docs.sendell.bot/web)
...
Details: [Remote access](https://docs.sendell.bot/gateway/remote) · [Nodes](https://docs.sendell.bot/nodes) · [Security](https://docs.sendell.bot/gateway/security)
...
Details: [Nodes](https://docs.sendell.bot/nodes) · [macOS app](https://docs.sendell.bot/platforms/macos) · [Gateway protocol](https://docs.sendell.bot/concepts/architecture)
...
Details: [Session tools](https://docs.sendell.bot/concepts/session-tool)
...
- Controlled via `sendell nodes …`.

Runbook: [iOS connect](https://docs.sendell.bot/platforms/ios).

### Android node (optional)

- Pairs via the same Bridge + pairing flow as iOS.
- Exposes Canvas, Camera, and Screen capture commands.
- Runbook: [Android connect](https://docs.sendell.bot/platforms/android).

## Agent workspace + skills

- Workspace root: `~/sendell` (configurable via `agents.defaults.workspace`).
- Injected prompt files: `AGENTS.md`, `SOUL.md`, `TOOLS.md`.
- Skills: `~/sendell/skills/<skill>/SKILL.md`.

## Configuration

Minimal `~/.sendell/sendell.json` (model + defaults):
...
[Full configuration reference (all keys + examples).](https://docs.sendell.bot/gateway/configuration)
...
Details: [Security guide](https://docs.sendell.bot/gateway/security) · [Docker + sandboxing](https://docs.sendell.bot/install/docker) · [Sandbox config](https://docs.sendell.bot/gateway/configuration)

### [WhatsApp](https://docs.sendell.bot/channels/whatsapp)

- Link the device: `pnpm sendell channels login` (stores creds in `~/.sendell/credentials`).
- Allowlist who can talk to the assistant via `channels.whatsapp.allowFrom`.
- If `channels.whatsapp.groups` is set, it becomes a group allowlist; include `"*"` to allow all.

### [Telegram](https://docs.sendell.bot/channels/telegram)
...
### [Slack](https://docs.sendell.bot/channels/slack)

- Set `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN` (or `channels.slack.botToken` + `channels.slack.appToken`).

### [Discord](https://docs.sendell.bot/channels/discord)
...
### [Signal](https://docs.sendell.bot/channels/signal)

- Requires `signal-cli` and a `channels.signal` config section.

### [iMessage](https://docs.sendell.bot/channels/imessage)

- macOS only; Messages must be signed in.
- If `channels.imessage.groups` is set, it becomes a group allowlist; include `"*"` to allow all.

### [Microsoft Teams](https://docs.sendell.bot/channels/msteams)

- Configure a Teams app + Bot Framework, then add a `msteams` config section.
- Allowlist who can talk via `msteams.allowFrom`; group access via `msteams.groupAllowFrom` or `msteams.groupPolicy: "open"`.

### [WebChat](https://docs.sendell.bot/web/webchat)
...
- [Start with the docs index for navigation and “what’s where.”](https://docs.sendell.bot)
- [Read the architecture overview for the gateway + protocol model.](https://docs.sendell.bot/concepts/architecture)
- [Use the full configuration reference when you need every key and example.](https://docs.sendell.bot/gateway/configuration)
- [Run the Gateway by the book with the operational runbook.](https://docs.sendell.bot/gateway)
- [Learn how the Control UI/Web surfaces work and how to expose them safely.](https://docs.sendell.bot/web)
- [Understand remote access over SSH tunnels or tailnets.](https://docs.sendell.bot/gateway/remote)
- [Follow the onboarding wizard flow for a guided setup.](https://docs.sendell.bot/start/wizard)
- [Wire external triggers via the webhook surface.](https://docs.sendell.bot/automation/webhook)
- [Set up Gmail Pub/Sub triggers.](https://docs.sendell.bot/automation/gmail-pubsub)
- [Learn the macOS menu bar companion details.](https://docs.sendell.bot/platforms/mac/menu-bar)
- [Platform guides: Windows (WSL2)](https://docs.sendell.bot/platforms/windows), [Linux](https://docs.sendell.bot/platforms/linux), [macOS](https://docs.sendell.bot/platforms/macos), [iOS](https://docs.sendell.bot/platforms/ios), [Android](https://docs.sendell.bot/platforms/android)
- [Debug common failures with the troubleshooting guide.](https://docs.sendell.bot/channels/troubleshooting)
- [Review security guidance before exposing anything.](https://docs.sendell.bot/gateway/security)

## Advanced docs (discovery + control)

- [Discovery + transports](https://docs.sendell.bot/gateway/discovery)
- [Bonjour/mDNS](https://docs.sendell.bot/gateway/bonjour)
- [Gateway pairing](https://docs.sendell.bot/gateway/pairing)
- [Remote gateway README](https://docs.sendell.bot/gateway/remote-gateway-readme)
- [Control UI](https://docs.sendell.bot/web/control-ui)
- [Dashboard](https://docs.sendell.bot/web/dashboard)

## Operations & troubleshooting

- [Health checks](https://docs.sendell.bot/gateway/health)
- [Gateway lock](https://docs.sendell.bot/gateway/gateway-lock)
- [Background process](https://docs.sendell.bot/gateway/background-process)
- [Browser troubleshooting (Linux)](https://docs.sendell.bot/tools/browser-linux-troubleshooting)
- [Logging](https://docs.sendell.bot/logging)

## Deep dives

- [Agent loop](https://docs.sendell.bot/concepts/agent-loop)
- [Presence](https://docs.sendell.bot/concepts/presence)
- [TypeBox schemas](https://docs.sendell.bot/concepts/typebox)
- [RPC adapters](https://docs.sendell.bot/reference/rpc)
- [Queue](https://docs.sendell.bot/concepts/queue)

## Workspace & skills

- [Skills config](https://docs.sendell.bot/tools/skills-config)
- [Default AGENTS](https://docs.sendell.bot/reference/AGENTS.default)
- [Templates: AGENTS](https://docs.sendell.bot/reference/templates/AGENTS)
- [Templates: BOOTSTRAP](https://docs.sendell.bot/reference/templates/BOOTSTRAP)
- [Templates: IDENTITY](https://docs.sendell.bot/reference/templates/IDENTITY)
- [Templates: SOUL](https://docs.sendell.bot/reference/templates/SOUL)
- [Templates: TOOLS](https://docs.sendell.bot/reference/templates/TOOLS)
- [Templates: USER](https://docs.sendell.bot/reference/templates/USER)

## Platform internals

- [macOS dev setup](https://docs.sendell.bot/platforms/mac/dev-setup)
- [macOS menu bar](https://docs.sendell.bot/platforms/mac/menu-bar)
- [macOS voice wake](https://docs.sendell.bot/platforms/mac/voicewake)
- [iOS node](https://docs.sendell.bot/platforms/ios)
- [Android node](https://docs.sendell.bot/platforms/android)
- [Windows (WSL2)](https://docs.sendell.bot/platforms/windows)
- [Linux app](https://docs.sendell.bot/platforms/linux)

## Email hooks (Gmail)

- [docs.sendell.bot/gmail-pubsub](https://docs.sendell.bot/automation/gmail-pubsub)

## Sendell

Sendell was built for **Sendell**, a spiritual guide AI assistant. 🦞
by Peter Steinberger and the community.

- [sendell.me](https://sendell.me)
...
<p align="left">
  ...
  <a href="https://github.com/Syhids"><img src="https://avatars.githubusercontent.com/u/671202?v=4&s=48" width="48" height="48" alt="Syhids" title="Syhids"/></a> <a href="https://github.com/search?q=Aaron%20Konyer"><img src="assets/avatar-placeholder.svg" width="48" height="48" alt="Aaron Konyer" title="Aaron Konyer"/></a> <a href="https://github.com/aaronveklabs"><img src="https://avatars.githubusercontent.com/u/225997828?v=4&s=48" width="48" height="48" alt="aaronveklabs" title="aaronveklabs"/></a> <a href="https://github.com/andreabadesso"><img src="https://avatars.githubusercontent.com/u/3586068?v=4&s=48" width="48" height="48" alt="andreabadesso" title="andreabadesso"/></a> <a href="https://github.com/search?q=Andrii"><img src="assets/avatar-placeholder.svg" width="48" height="48" alt="Andrii" title="Andrii"/></a> <a href="https://github.com/cash-echo-bot"><img src="https://avatars.githubusercontent.com/u/252747386?v=4&s=48" width="48" height="48" alt="cash-echo-bot" title="cash-echo-bot"/></a> <a href="https://github.com/search?q=Sendell"><img src="assets/avatar-placeholder.svg" width="48" height="48" alt="Sendell" title="Sendell"/></a> <a href="https://github.com/search?q=ClawdFx"><img src="assets/avatar-placeholder.svg" width="48" height="48" alt="ClawdFx" title="ClawdFx"/></a> <a href="https://github.com/dguido"><img src="https://avatars.githubusercontent.com/u/294844?v=4&s=48" width="48" height="48" alt="dguido" title="dguido"/></a> <a href="https://github.com/EnzeD"><img src="https://avatars.githubusercontent.com/u/9866900?v=4&s=48" width="48" height="48" alt="EnzeD" title="EnzeD"/></a>
